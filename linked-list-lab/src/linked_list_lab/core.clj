...
(cond (empty? xx) nil
      (= elmt (:car xx))(:cdr xx)
      :else (Cons. (:car xx) (delete-helper elmt (:cdr xx)))))

(defn delete
  "Delete an element elt from xx"
 [elmt data]
  (let [ndata (delete-helper elmt (:data data))
        nsize (count (cons-to-list ndata))]
       (List. ndata nsize)))
...
(defn delete-all-helper
  "This is used by delete-all"
  [elmt xx]
  (cond (empty? xx) nil
        (= elmt (:car xx)) (delete-all-helper elmt (:cdr xx))
        :else (Cons. (:car xx) (delete-all-helper elmt (:cdr xx)))))

(defn delete-all
  "Delete all occurrences of elt from xx"
  [elmt data]
  (let [ndata (delete-all-helper elmt (:data data))
        nsize (count (cons-to-list ndata))]
       (List. ndata nsize)))
