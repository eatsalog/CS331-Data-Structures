...
(facts "about `delete`"
  (let [nlist (List. (Cons. 1 (Cons. 7 (Cons. 3 (Cons. 5 nil)))) 4)]
    (fact "It deletes elements properly"
      (-> (delete 1 nlist) :data) => (list-to-cons '(7 3 5))
      (-> (delete 7 nlist) :data) => (list-to-cons '(1 3 5))
      (-> (delete 5 nlist) :data) => (list-to-cons '(1 7 3))
      (-> (delete 4 nlist) :data) => (list-to-cons '(1 7 3 5)))
    (fact "It decrements size properly"
      (-> (delete 7 nlist) :size) => 3
      (-> (delete 4 nlist) :size) => 4)))

(facts "about `delete-all`"
  (let [nul (List. (Cons. 7 (Cons. 1 (Cons. 3 (Cons. 1 (Cons. 7 (Cons. 4 (Cons. 7 nil))))))) 7)]
    (fact "It deletes elements properly"
      (-> (delete-all 7 nul) :data) => (list-to-cons '(1 3 1 4))
      (-> (delete-all 1 nul) :data) => (list-to-cons '(7 3 7 4 7))
      (-> (delete-all 3 nul) :data) => (list-to-cons '(7 1 1 7 4 7))
      (-> (delete-all 5 nul) :data) => (list-to-cons '(7 1 3 1 7 4 7)))
    (fact "It decrements size properly"
      (-> (delete-all 7 nul) :size) => 4
      (-> (delete-all 1 nul) :size) => 5
      (-> (delete-all 5 nul) :size) => 7)))
